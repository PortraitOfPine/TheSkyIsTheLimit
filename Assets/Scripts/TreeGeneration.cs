...
public class TreeGeneration : MonoBehaviour
{
    ...
    [SerializeField] private NoiseGeneration.Wave[] waves;
    ...
    public void GenerateTrees(int levelDepth, int levelWidth, float distanceBetweenVertices, LevelData levelData)
    {
        ...
        float[,] treeMap =
            this.noiseMapGeneration.GenerateNoiseMap(levelDepth, levelWidth, levelScale, 0, 0, this.waves);
        ...
        for (int zIndex = 0; zIndex < levelDepth; zIndex++)
        {
            for (int xIndex = 0; xIndex < levelWidth; xIndex++)
            {
                ...
                TerrainType terrainType = tileData.chosenHeightTerrainTypes[tileCoordinate.coordinateZIndex,
                ...
                if (terrainType.name != "water")
                {
                    ...
                    {
                        Vector3 treePosition = new Vector3((xIndex-5) * distanceBetweenVertices,
                            meshVertices[vertexIndex].y, (zIndex-5) * distanceBetweenVertices);
                        GameObject tree = Instantiate(this.treePrefab, this.transform.position + treePosition, Quaternion.identity) as GameObject;
